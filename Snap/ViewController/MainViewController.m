...
- (void)clipIntroductionButton
{
    CGRect buttonFrame = self.introduceButton.frame;
    self.introduceButton.layer.cornerRadius = buttonFrame.size.width / 2.0;
    self.introduceButton.layer.masksToBounds = YES;
}
...
- (IBAction)hostGameAction:(id)sender
{
	if (_buttonsEnabled)
	{
		[self performExitAnimationWithCompletionBlock:^(BOOL finished)
         {	
             HostViewController *controller = [[HostViewController alloc]
                                               initWithNibName:@"HostViewController"
                                               bundle:nil];
...
- (IBAction)joinGameAction:(id)sender
{
	if (_buttonsEnabled)
	{
		[self performExitAnimationWithCompletionBlock:^(BOOL finished)
         {
             JoinViewController *controller = [[JoinViewController alloc]
                                               initWithNibName:@"JoinViewController"
...
- (IBAction)showIntroductionAction:(id)sender
{
    if (_buttonsEnabled) {
        [self performExitAnimationWithCompletionBlock:^(BOOL finished)
        {
            IntroduceViewController *controller = [[IntroduceViewController alloc]
                                                   initWithNibName:@"IntroduceViewController"
                                                   bundle:nil];
            controller.delegate = self;
...
- (void)prepareForIntroAnimation
{
    ...
    self.introduceButton.alpha = 0.0f;
...
- (void)performIntroAnimation
{
    ...
    [UIView animateWithDuration:0.5f
                     ...
                     animations:^
     {
         ...
         self.introduceButton.alpha = 1.0f;
     }
	...
	[UIView animateWithDuration:0.3f
                     ...
                     animations:^
     {
         ...
         self.jokerImageView.transform = self.aImageView.transform;
                     ...
                     completion:^(BOOL finished)
     {
         CGPoint point = CGPointMake(self.aImageView.center.x,
...
- (void)showNoNetworkAlert
{
	UIAlertView *alertView = [[UIAlertView alloc] 
                              initWithTitle:NSLocalizedString(@"没有网络", @"No network alert title")
                              message:NSLocalizedString(@"为了能够多人游戏,请打开设备上的WiFi或者蓝牙以连接其它设备.", @"No network alert message")
                              delegate:nil
...
{
	UIAlertView *alertView = [[UIAlertView alloc] 
                              initWithTitle:NSLocalizedString(@"失去连接", @"Client disconnected alert title")
                              message:NSLocalizedString(@"你与游戏失去连接.", @"Client disconnected alert message")
                              delegate:nil
                              cancelButtonTitle:NSLocalizedString(@"确定", @"Button: OK")
                              otherButtonTitles:nil];
